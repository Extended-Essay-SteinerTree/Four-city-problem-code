...
def ABCDnames_STEP0(x,y):
    ...
    t4=main_fig.text(x[3]+4, y[3]-10, "D")
...
lineAB=draw_line([x_initial[0],y_initial[0]],[x_initial[1],y_initial[1]])
lineBC=draw_line([x_initial[1],y_initial[1]],[x_initial[2],y_initial[2]])
lineCD=draw_line([x_initial[2],y_initial[2]],[x_initial[3],y_initial[3]])

# calculate total_distance
dist_AB=calculate_distance(x_initial[0],y_initial[0],x_initial[1],y_initial[1])/3
dist_BC=calculate_distance(x_initial[1],y_initial[1],x_initial[2],y_initial[2])/3
dist_CD=calculate_distance(x_initial[2],y_initial[2],x_initial[3],y_initial[3])/3
print(dist_AB, dist_BC, dist_CD)
total_distance=dist_AB + dist_BC + dist_CD
